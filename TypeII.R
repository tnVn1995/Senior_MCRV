...
generate_cluster = function(probw, probyw1, probyw2, Uw, Uy1, Uy2, i, pos1, pos2, row, data=NULL){
  ...
  if (Uw >= probw[1,3] & Uw < probw[2,3]){
    ...
    data[i, 1] = 1
    data[i, 2] = 0
    data = generate_y(probyw1, data, Uy1,row=i, pos1)
  }
  else if (Uw >= probw[3,3] & Uw < probw[4,3]){
    data[i, 1] = 1
    data[i, 2] = 1
    data = generate_y(probyw1, data, Uy1, row=i, pos1)
    data = generate_y(probyw2, data, Uy2, row=i, pos2)
  }
  else {
    data[i,1] = 0
    data[i,2] = 1
...
ptime = system.time({r <- foreach(icount(trials), .combine=rbind) %dopar% {
  ...
  Y3 = sample(x=which(!is.na(data[,3])),size=n,replace=TRUE)
  Y4 = sample(x=which(!is.na(data[,4])),size=n,replace=TRUE)
  c3 = data[Y3,3]
  c4 = data[Y4,4]
  c5 = data[Y3,5]
  c6 = data[Y4,6]
